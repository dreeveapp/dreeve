...
class FindRouteMatchesQueryHandlerTest extends ContainerTestCase
{
    ...
    public function testHandleDoesNotMatchARouteThatDivergesAtASingleWaypoint(): void
    {
        $route = range(1, 20);
        $forward = ActivityRouteSignatureBuilder::fromDefaults()->build()->getWaypoints()->toArray();
        // Six waypoints land on top of each other and one sits 4 km away: a median of seven
        // reported no drift at all for this, so the detour was listed as a similar route.
        $detoured = $forward;
        $detoured[6] += 3600;

        $this->addActivityWithRouteCells('subject', $route);
        $this->addActivityWithRouteCells('detoured', $route, waypoints: RouteWaypoints::fromArray($detoured));
    ...
    public function testHandleDoesNotMatchALongRouteThatDriftsMoreThanTheAbsoluteMaximum(): void
    {
        $route = range(1, 20);
        $forward = ActivityRouteSignatureBuilder::fromDefaults()->build()->getWaypoints()->toArray();
        $drifted = array_map(fn (int $coordinate, int $index): int => 0 === $index % 2 ? $coordinate + 2500 : $coordinate, $forward, array_keys($forward));
