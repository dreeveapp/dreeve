...
final readonly class ElevationCondition implements Condition
{
    ...
    {
        return $translator->trans('Elevation', domain: 'admin', locale: $locale);
    }
    ...
    public function describeValue(TranslatorInterface $translator, RuleConfiguration $configuration): string
    {
        return sprintf(
            ...
            $this->settingsRepository->appearance()->getUnitSystem()->elevationSymbol(),
        );
    }
    ...
    {
        return 'automation-condition--elevation';
    }
    ...
    public function guardValidConfiguration(RuleConfiguration $configuration): void
    {
        ...
        if (!is_string($operator) || null === ComparisonOperator::tryFrom($operator)) {
            throw new InvalidAutomationRule(sprintf('Invalid elevation operator "%s".', is_scalar($operator) ? (string) $operator : ''));
        }
    ...
    public function matches(Activity $activity, RuleConfiguration $configuration): bool
    {
        return ComparisonOperator::from($configuration->getString('operator'))->isSatisfiedBy(
            actual: $activity->getElevation()->toUnitSystem($this->settingsRepository->appearance()->getUnitSystem())->toFloat(),
