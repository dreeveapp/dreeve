...
final readonly class AverageCadenceCondition implements Condition
{
    public function trans(TranslatorInterface $translator, ?string $locale = null): string
    {
        return $translator->trans('Average cadence', domain: 'admin', locale: $locale);
    }
    ...
    {
        return 'automation-condition--average-cadence';
    }
    ...
    public function guardValidConfiguration(RuleConfiguration $configuration): void
    {
        ...
        if (!is_string($operator) || null === ComparisonOperator::tryFrom($operator)) {
            throw new InvalidAutomationRule(sprintf('Invalid average cadence operator "%s".', is_scalar($operator) ? (string) $operator : ''));
        }
