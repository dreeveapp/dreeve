...
- A Strava API application (create one at
  [https://www.strava.com/settings/api](https://www.strava.com/settings/api)) if importing via the Strava API
...
```yaml title="dreeve.yaml" mark=/CHANGE-ME/
---
apiVersion: v1
kind: Namespace
metadata:
...
stringData:
  ...
  # Optional. Enables the activity upload API; omit it to keep the API closed.
  # DREEVE_API_KEY: "drv_..."
---
