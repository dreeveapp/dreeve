...
`stravaApi` mode. If you don't have a subscription, use the default [`files` mode](/importing/file-import/) instead.
:::
...
:::note[Important]
Do not include a port number in the _Authorization Callback Domain_ field.
:::
...
:::danger
Caution Do **not** use the refresh token displayed on your Strava API settings page, it will not work.
:::
