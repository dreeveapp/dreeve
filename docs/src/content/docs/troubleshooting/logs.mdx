...
Each entry starts with a short reference that is also shown on the error page the visitor sees, so you can ask whoever reported the problem for that reference and search the log for it.
