...
:::note[Important]
This uses Garmin's unofficial API. Garmin does not document or support it and has
changed the authentication flow before, so expect occasional breakage. Read
[An unofficial API](/integrations/garmin-connect/#an-unofficial-api)
...
| `activities` | The whole ledger counted by [status](#commands).           |
